...
long wd_wandering; 
int wanderingTimeout = 60E3; // 1 minuto dando vueltas...
...
void loop()
{
  ...
  case WANDERING:        
    // watchdog
    if(millis() > wd_wandering + wanderingTimeout) state = LOOKING_FOR_MUM;
  ...
  case LOOKING_FOR_MUM:
    stop();
    if (status != B00011111) state = GO;      
